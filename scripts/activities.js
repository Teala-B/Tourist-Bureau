...
function init() {
    ...
    selectCategory.onchange = activityName

    let length = categories.length
    let option = document.createElement("option")
    option.textContent = "Seclect One"
    selectCategory.appendChild(option)
    for(let i = 0; i < length; i++){
        let option = document.createElement("option");
        option.textContent = categories[i];
        //option.value = categories[i].code
        selectCategory.appendChild(option)
    }
}
...
function activityName(event) {
    ...
    activityDetails.innerHTML = "";
        ...
        const activities = filterActivities(selectCategory.value)
...
function filterThroughActivitySet(params) {
...
function displayActivityInfo(event) {
    ...
    const activities = filterActivities(selectCategory.value)
    for(let item of activities){
        const option = new Option(item.name, item.description, item.location, item.price);
        activityDetails.appendChild(option)
    }
}